...
/*
    ...
    3) 서블릿 컨테이너가 요청을 처리
        - 요청 URL을 보고 web.xml 또는 어노테아션(@WebServlet)을 통해
            어떤 서블릿이 처리할지 결정
    ...
    5) 서블릿이 요청을 처리(servlet -> doGot / doPost)
    ...
    7) 웹 서버가 클라이언트에게 응답 전송
        : 후처리 필터까지 모두 끝나면 HttpServletResponse 내용을 Http 응답 형태로 반환하여 브라우저에 전달

    8) 서블릿 종료 (destroy)
        - 서버가 종료되서나 서블릿 제거시 destroy()호출
